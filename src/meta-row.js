...
(function(_global) {
            ...
            function getPropDescriptor(p_idx) {
                return {
                    ...
                    set: function(nVal) { 
                        ...
                        if (column && column._valueTypes.length > 0) Util.typeValid(column._valueTypes, nVal);
                        // 트렌젹션 처리 => 함수로 추출 검토
    ...
    var MetaRowCollection  = (function (_super) {
         ...
         * @param {MetaRow} p_row 
         * @param {boolean?} p_typeValid true: 검사 진행, false: 검사 안함
         * @returns {number}
         */
        MetaRowCollection.prototype.add  = function(p_row, p_typeValid) {
            return this.insertAt(this._elements.length, p_row, p_typeValid);
        };
         ...
         * @param {number} p_pos 
         * @param {MetaRow} p_row 
         * @param {boolean?} p_typeValid 유효성 검사 여부 
         * @returns {boolean}
         */
        MetaRowCollection.prototype.insertAt  = function(p_pos, p_row, p_typeValid) {
            var typeValid = p_typeValid || false;
            var result;
